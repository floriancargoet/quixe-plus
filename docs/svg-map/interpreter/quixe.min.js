...
// QuixeProbe addition
self.getConstantString = function (addr) {
    // check addr type
    const type = Mem1(addr);
    // function = 0xC0 or 0xC1
    // string  = 0xE1 or 0xE0 or 0xE2
    if (type !== 0xE1 && type !== 0xE0 && type !== 0xE2) {
        /*
        const _Glk = window.Glk;
        window.Glk = window.FakeGlk;
        vm_autosave();
        try {
            enter_function(addr, 0);
            self.done_executing = self.vm_stopped;
            execute_loop();
        }
        catch (e) {
        ...
        window.Glk = _Glk;
        return; */
        console.error("String is not a constant");
        return "";
    }
    let strop = compile_string(self.iosysmode, addr, 0, 0);
    if (!(strop instanceof Function)) {
        return strop;
    }
    throw new Error("Unsupported string function");
}
